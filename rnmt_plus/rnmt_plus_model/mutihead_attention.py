...
from rnmt_plus_model.keyvalue_attention import KeyValAttention
...
class MultiHeadAttention(nn.Module):
    ...
    def forward_3d(self, query, keys, values, mask=None):
        ...
        context_vectors, weights = self._attention(query, keys, values, mask=mask,
                                                   additional_logits=relpos_logits)  # (B, 4, T2, H)
        context_vectors = context_vectors.transpose(1, 2).contiguous().view(B, -1,
                                                                            self._num_head * head_dim)  # (B, T2, H)
